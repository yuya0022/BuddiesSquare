<section>
    ...
    <div>
        <x-input-label :value="__('【必須】　プロフィール写真（メイン）')" />
        
        <!--メイン写真表示-->
        <img src="{{ $user->main_image }}" style="max-width:200px;">
        
        <!--変更する-->
        <form action="/main_image" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <input type="file" name="main_image" accept='image/*' onchange="submit(this.form)">
        </form>
    </div>
    ...
    @for($i = 0; $i < count($sub_images); $i++)
        ...
        <!--変更する-->
        <form action="/sub_images/{{ $sub_images[$i]->id }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <input type="file" name="sub_image" accept='image/*' onchange="submit(this.form)">
        </form>
        
        <!--削除する-->
        <form action="/sub_images/{{ $sub_images[$i]->id }}" id="form_{{ $sub_images[$i]->id }}" method="POST">
            @csrf
            @method('DELETE')
            <button type="button" onclick="deleteSubImage({{ $sub_images[$i]->id }})">削除する</button>
        </form>
    @endfor
    @for($i = count($sub_images); $i < 9; $i++)
        ...
        <!--追加する-->
        <form action="/sub_images" method="POST" enctype="multipart/form-data">
            @csrf
            <input type="file" name="sub_image" accept='image/*' onchange="submit(this.form)">
        </form>
    @endfor
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        　　　　<input id="sex" type="radio" name="sex" value="0" @if( $user->sex === 0 ) checked @endif disabled>女性
           　　　<input id="sex" type="radio" name="sex" value="1" @if( $user->sex === 1 ) checked @endif disabled>男性
            ...
            <div>
                ...
                <textarea id="self-introduction" name="self-introduction"> {{ old('self-introduction', $user['self-introduction']) }} </textarea>
        ...
        <div>
            ...
            <div>
                ...
                <textarea id="answer_1" name="answer_1">{{ old('answer_1', $answer->answer_1) }}</textarea>
            ...
            <div>
                ...
                <textarea id="answer_2" name="answer_2">{{ old('answer_2', $answer->answer_2) }}</textarea>
            ...
            <div>
                ...
                <textarea id="answer_3" name="answer_3">{{ old('answer_3', $answer->answer_3) }}</textarea>
            ...
            <div>
                ...
                <textarea id="answer_4" name="answer_4">{{ old('answer_4', $answer->answer_4) }}</textarea>
            ...
            <div>
                ...
                <textarea id="answer_5" name="answer_5">{{ old('answer_5', $answer->answer_5) }}</textarea>
            ...
            <div>
                ...
                <textarea id="answer_6" name="answer_6">{{ old('answer_6', $answer->answer_6) }}</textarea>
